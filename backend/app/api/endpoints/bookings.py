...
async def create_booking_request(request: BookingRequest, current_user: dict = Depends(get_current_user)) -> BookingRequest:
    """
...
async def get_mentor_bookings(mentor_id: str, current_user: dict = Depends(get_current_user)) -> List[dict]:
    """
...
async def get_mentee_bookings(mentee_id: str, current_user: dict = Depends(get_current_user)) -> List[dict]:
    """
...
async def update_booking_status(booking_id: str, status: str, current_user: dict = Depends(get_current_user)) -> dict:
    """
    Update the status of a booking (accepted, rejected), ensuring only the mentor can do so.
